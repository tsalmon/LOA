...
//  Projet
//
//  Created by Владислав Фиц on 19.01.14.
...
void showMatrix(int **m, int nbLignes, int nbColonnes){
    for (int i = 0; i<nbLignes; ++i) {
        for (int j = 0; j<nbColonnes; ++j) {
            cout<<m[i][j]<<" ";
        }
...
void checkFive(Plateau *p){
    int nbLignes = p->get_sizeX();
    int nbColonnes = p->get_sizeY();
    int **hm = getHorizontalMask(p, nbLignes, nbColonnes);
    int **vm = getVerticalMask(p, nbLignes, nbColonnes);
    int **dm1 = getDiagonalMask1(p, nbLignes, nbColonnes);
    int **dm2 = getDiagonalMask2(p, nbLignes, nbColonnes);
